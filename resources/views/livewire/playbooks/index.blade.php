<div>
    <x-header title="Playbooks" separator progress-indicator>
        ...
        <x-slot:actions>
            <x-button label="Create" icon="o-plus" link="{{ route('workbench.playbooks.create') }}" class="btn-primary" />
        </x-slot:actions>
    </x-header>
        ...
        <x-table :headers="$headers" :rows="$playbooks" :sort-by="$sortBy" with-pagination>
            ...
            @scope('actions', $playbook)
                <div class="flex gap-1">
                    <x-button icon="o-pencil" link="{{ route('workbench.playbooks.edit', $playbook->id) }}" spinner class="btn-ghost btn-sm" />
