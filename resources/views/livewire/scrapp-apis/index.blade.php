<div>
    <x-header title="Scrapp APIs" separator progress-indicator>
        ...
        <x-slot:actions>
            <x-button label="Create" icon="o-plus" link="{{ route('app.scrapp-apis.create') }}" class="btn-primary" />
        </x-slot:actions>
    </x-header>
    ...
    <x-card shadow>
        <x-table :headers="$headers" :rows="$scrappApis" :sort-by="$sortBy" with-pagination>
            ...
            @scope('actions', $api)
                <x-button icon="o-pencil" link="{{ route('app.scrapp-apis.edit', $api->id) }}" spinner class="btn-ghost btn-sm" />
