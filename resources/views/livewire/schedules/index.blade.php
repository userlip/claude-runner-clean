<div>
    <x-header title="Task Schedules" separator progress-indicator>
        ...
        <x-slot:actions>
            <x-button label="Create" icon="o-plus" link="{{ route('app.schedules.create') }}" class="btn-primary" />
        </x-slot:actions>
    </x-header>
    ...
    <x-card shadow>
        <x-table :headers="$headers" :rows="$schedules" :sort-by="$sortBy" with-pagination>
            ...
            @scope('actions', $schedule)
                <x-button icon="o-pencil" link="{{ route('app.schedules.edit', $schedule->id) }}" spinner class="btn-ghost btn-sm" />
