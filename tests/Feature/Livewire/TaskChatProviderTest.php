...
test('it can change provider', function () {
    ...
    $kimi = AiProvider::factory()->kimi()->create(['is_active' => true]);
    $task = Task::factory()->create(['ai_provider_id' => $claude->id]);

    Livewire::actingAs($user)
        ->test(TaskChat::class, ['task' => $task])
        ->call('setProvider', $kimi->id)
        ->assertSet('task.ai_provider_id', $kimi->id);

    expect($task->fresh()->ai_provider_id)->toBe($kimi->id);
});
