...
function getUpstreamBinaryPath(): string {
  return (
    process.env.GSC_MCP_SERVER_PATH ||
...
function getTools(accountConfigs: AccountConfig[]): Tool[] {
  ...
  return [
    ...
    },
    {
      name: "gsc-list-sites",
      description:
        "List all sites (properties) in Google Search Console accessible by the specified account.",
      inputSchema: {
        type: "object" as const,
        properties: {
    ...
    },
    {
      ...
      description:
        "Get search performance data (clicks, impressions, CTR, position) from Google Search Console. Supports filtering by page, query, country, device.",
      inputSchema: {
        type: "object" as const,
        properties: {
          account: accountParam,
          siteUrl: {
            type: "string" as const,
            description:
              "The site URL as registered in Search Console (e.g. 'https://example.com/' or 'sc-domain:example.com').",
          },
          startDate: {
            type: "string" as const,
          ...
          dimensions: {
            type: "string" as const,
            description:
              "Comma-separated dimensions to group by: query, page, country, device, searchAppearance, date. Example: 'query,page'.",
          },
          type: {
            type: "string" as const,
            description:
    ...
    },
    {
      ...
      description:
        "Enhanced search analytics with up to 25,000 rows, regex filters, and optional quick wins detection.",
      inputSchema: {
        type: "object" as const,
        properties: {
          account: accountParam,
          ...
          pageFilter: { type: "string" as const },
          queryFilter: { type: "string" as const },
          countryFilter: { type: "string" as const },
          deviceFilter: { type: "string" as const },
          filterOperator: filterOperatorEnum,
          aggregationType: { type: "string" as const },
          regexFilter: {
            type: "string" as const,
          ...
          enableQuickWins: {
            ...
            description:
              "Enable quick wins detection in the response.",
          },
          quickWinsThresholds: {
            type: "object" as const,
            description:
              "Custom thresholds for quick wins detection (minImpressions, maxCtr, positionRangeMin, positionRangeMax).",
          },
        },
        required: accountConfigs.length > 1
          ? ["account", "siteUrl", "startDate", "endDate"]
    ...
    },
    {
      ...
      description:
        "Automatically detect SEO quick wins and optimization opportunities based on search analytics data.",
      inputSchema: {
        type: "object" as const,
        properties: {
          account: accountParam,
          ...
          maxCtr: {
            type: "number" as const,
            description:
              "Maximum CTR threshold (default: 0.05 = 5%).",
          },
          positionRangeMin: {
            type: "number" as const,
            description:
              "Minimum average position (default: 3).",
          },
          positionRangeMax: {
            type: "number" as const,
            description:
              "Maximum average position (default: 20).",
          },
        },
        required: accountConfigs.length > 1
          ? ["account", "siteUrl", "startDate", "endDate"]
    ...
    },
    {
      ...
      description:
        "Inspect a URL to check its indexing status, crawl info, and rich results in Google Search.",
      inputSchema: {
        type: "object" as const,
        properties: {
          account: accountParam,
          ...
          languageCode: {
            type: "string" as const,
            description:
              "Language code for the inspection (e.g. 'en-US').",
          },
        },
        required: accountConfigs.length > 1
    ...
    },
    {
      ...
      description:
        "Get details of a specific sitemap for a site in Google Search Console.",
      inputSchema: {
        type: "object" as const,
        properties: {
          account: accountParam,
