...
export const GSCBaseSchema = z.object({
    siteUrl: z
        .string()
        .describe('The site URL as defined in Search Console. Example: sc-domain:example.com (for domain resources) or http://www.example.com/ (for site prefix resources)'),
});
export const SearchAnalyticsSchema = GSCBaseSchema.extend({
    ...
    dimensions: z
        ...
        .refine((val) => val.every((d) => ['query', 'page', 'country', 'device', 'date', 'searchAppearance'].includes(d)))
        .optional()
        .describe('Comma-separated list of dimensions to break down results by, such as query, page, country, device, date, searchAppearance'),
    type: z
...
export const IndexInspectSchema = GSCBaseSchema.extend({
    inspectionUrl: z
        .string()
        .describe('The fully-qualified URL to inspect. Must be under the property specified in "siteUrl"'),
    languageCode: z
        .string()
        .optional()
        .default('en-US')
        .describe('An IETF BCP-47 language code representing the language of the requested translated issue messages, such as "en-US" or "de-CH". Default is "en-US"'),
});
export const ListSitemapsSchema = z.object({
    sitemapIndex: z
        .string()
        .optional()
        .describe("A URL of a site's sitemap index. For example: http://www.example.com/sitemapindex.xml"),
    siteUrl: z
        .string()
        .optional()
